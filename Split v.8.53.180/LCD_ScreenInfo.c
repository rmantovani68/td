...
//            sprintf(LCDstr, "DefMode= %i BRC=%i          ", Me.DefMode, Me.BroadcastRxCounter);
            sprintf(LCDstr, "DefMode= %i           ", Me.DefMode);			
            I2C_LCD_Out(1,1,LCDstr);     
            sprintf(LCDstr, "FuncMode= %i           ", Me.Function_Mode);
            I2C_LCD_Out(2,1,LCDstr);     
            sprintf(LCDstr, "FanSpeed= %i           ", Me.Ventil_Mode);
            I2C_LCD_Out(3,1,LCDstr);     			
            sprintf(LCDstr, "SP= %i T=%i               ", Me.SP, Me.CycleTime);
...
//                           12345678901234567890       
 //           sprintf(LCDstr, "TDefMode= %i BRC=%i         ", Touch[0].DefMode, Me.BroadcastRxCounter);
           sprintf(LCDstr, "TDefMode= %i          ", Touch[0].DefMode);			
            I2C_LCD_Out(1,1,LCDstr);     
            sprintf(LCDstr, "TFuncMode= %i           ", Touch[0].FunctionMode);
            I2C_LCD_Out(2,1,LCDstr);     
            sprintf(LCDstr, "TFanSpeed= %i           ", Touch[0].FanMode);
            I2C_LCD_Out(3,1,LCDstr);     			
            sprintf(LCDstr, "TSP= %i   PWR=%i        ", Touch[0].SetPoint, Touch[0].Script_Split_Pwr);
            I2C_LCD_Out(4,1,LCDstr);                  // Write text in fourth row				
			/*
...
//                           00000000011111111112
//                           12345678901234567890                
            sprintf(LCDstr, "Temp_Amb= %.2f 'C     ", (float)Me.Temperature.Ambient/100.0);
